...
Since, our datasets are very big, it would computationally expensive to slice the data and obtain subsets each time we create a new visualization. Hence, we need to create a database on SQLite3 in order to facilitate easy access to subsets of our data. 
...
# Extra Plots on the Dataframe

```python
sns.lineplot(data = avg_temps, x = "Decade",  y = "Temp")
```

![title](/images/output_27_1.png) 
